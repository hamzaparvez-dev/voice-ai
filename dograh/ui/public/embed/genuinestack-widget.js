/**
 * GenuineStack Voice Widget
 * Embeddable voice call widget for GenuineStack workflows
 * Version: 1.0.0
 */
...
(function() {
  ...
  async function init() {
    ...
    const script = document.currentScript || document.querySelector('script[src*="genuinestack-widget.js"]');
    if (!script) {
      console.error('GenuineStack Widget: Script not found');
      return;
    }
    ...
    if (!token) {
      console.error('GenuineStack Widget: No token found in script URL');
      return;
    }
    ...
    state.config = {
      ...
      contextVariables: parseContextVariables(
        script.getAttribute('data-genuinestack-context')
    ...
    try {
      ...
      state.config = {
        ...
        containerId: configData.settings?.containerId || 'genuinestack-inline-container',
      ...
      console.error('GenuineStack Widget: Failed to fetch configuration', error);
      return;
    }
  ...
  function parseContextVariables(contextStr) {
    ...
    try {
      ...
      console.warn('GenuineStack Widget: Invalid context variables', e);
      return {};
    }
  }
  ...
  /**
   * Create headless widget (no UI — host page drives everything via window.GenuineStackWidget API)
   */
  ...
  function createInlineWidget() {
    ...
    if (!container) {
      console.error(`GenuineStack Widget: Container element with id "${state.config.containerId}" not found`);
    ...
    // Clear container
    container.innerHTML = '';
    container.className = 'genuinestack-inline-container';
  ...
  async function startCall() {
    ...
    try {
      ...
      console.error('GenuineStack Widget: Failed to start call', error);
  ...
  async function fetchTurnCredentials() {
    if (!state.sessionToken) {
      console.warn('GenuineStack Widget: No session token available for TURN credentials');
      return;
    }
  ...
  window.GenuineStackWidget = {
    ...
    initInline: (options) => {
      if (options.container) {
        state.config.containerId = options.container.id || 'genuinestack-inline-container';
      }
